...
def extractClassFromOntology(sparql:SPARQLWrapper, uri_ontology:str, config_class:dict, mapping_information_relation:dict) -> dict: 
    ...
    class_to_retrieve = set()
    for c in extracted_information:
        for key in {"SubClassOf"}:
            if key in extracted_information[c]:
                for c_to_check in  extracted_information[c][key]:
                    if c_to_check[1:-1] not in extracted_information:
                        class_to_retrieve.add(c_to_check[1:-1])
    class_to_retrieve = list(class_to_retrieve)

    res_empty = False
    # class_to_retrieve = {c_to_check for c in extracted_information for key in {"SubClassOf"} for c_to_check in  extracted_information[c][key] if c_to_check not in extracted_information}
    while (len(class_to_retrieve) != 0) and (not res_empty):
        # print(class_to_retrieve)
        step = 100
        res_empty = True
        for i in range(0, len(class_to_retrieve), step):
            query_init = """
                PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
                PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
                SELECT DISTINCT *
                WHERE {
                    VALUES ?class {
                    <"""+"> <".join(class_to_retrieve[i:i+step])+""">
                    }
            ...
            for result in response["results"]["bindings"]:
                ...
                if "Label" not in extracted_information[class_uri]:
                    extracted_information[class_uri]["Label"] = set()
                extracted_information[class_uri]["Label"].add(curate_value(result["label"]))
            ...
            query_init = """
                ...
                WHERE {
                    ...
                    ?class rdfs:subClassOf ?upperClassOf.
            }
            """
            ...
            for result in response["results"]["bindings"]:
                ...
                if "SubClassOf" not in extracted_information[class_uri]:
                    extracted_information[class_uri]["SubClassOf"] = set()
                extracted_information[class_uri]["SubClassOf"].add(curate_value(result["upperClassOf"]))
