...
def retrieve_vocabularies(f_out, data):
    ...
    f_out.write(f'<{data["VocabularyIRI"]}> <http://purl.org/dc/terms/modified> "2023-12-01T12:00Z".\n')
    f_out.write(f'<{data["VocabularyIRI"]}> <http://purl.org/dc/terms/title> "Wikidata".\n')
    f_out.write(f'<{data["VocabularyIRI"]}> <http://www.w3.org/2000/01/rdf-schema#comment> "free knowledge graph hosted by Wikimedia and edited by volunteers".\n')
...
def retrieve_classes(f_out, data):
    ...
    with open(f"Result_query.json", 'r', encoding="UTF-8") as f_read:
        for record in ijson.items(f_read, "results.bindings.item"):
            ...
            f_out.write(f'<{class_sub}> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://www.w3.org/1999/02/22-rdf-syntax-ns#Class>.\n')
            f_out.write(f'<{class_sub}> <http://www.w3.org/2000/01/rdf-schema#isDefinedBy> <{data["VocabularyIRI"]}>.\n')

    query = """
        SELECT DISTINCT ?class ?label ?description
        WHERE {
        ?class <http://www.wikidata.org/prop/direct/P279> ?c_other.
