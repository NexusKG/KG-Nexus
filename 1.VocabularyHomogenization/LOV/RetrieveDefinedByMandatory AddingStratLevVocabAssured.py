...
def retrieve_properties(sparql:SPARQLWrapper, vocabularies):
    ...
    for relation_to_retrieve in interesting_relations_to_retrieve:
        ...
        response = sparql.queryAndConvert()
        for result in response["results"]["bindings"]: 
            if relation_to_retrieve != "http://www.w3.org/2000/01/rdf-schema#isDefinedBy" or\
                curate_value(result["valueRelation"])[1:-1] in vocabularies:
...
def retrieve_classes(sparql:SPARQLWrapper, vocabularies):
...
def adding_isDefinedBy(vocabularies, components):

    components_to_check = [k for k in components.keys() if ("http://www.w3.org/2000/01/rdf-schema#isDefinedBy" not in components[k])]
    components_to_reduce = [k for k in components.keys() if ("http://www.w3.org/2000/01/rdf-schema#isDefinedBy" in components[k]) and \
                            (len(components[k]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"])!=1)]
    # print(componentwc s_to_reduce)
    prefered_uris = {prefUris[1:-1]:f"<{vocab}>" for vocab in vocabularies for prefUris in vocabularies[vocab]["http://purl.org/vocab/vann/preferredNamespaceUri"] }
    
    for uri in components_to_check:
        uri_reduced = ""
        if "#" in uri:
            uri_reduced = "#".join(uri.split("#")[:-1])+"#"
        else:
            uri_reduced = "/".join(uri.split("/")[:-1])+"/"

        if uri_reduced in prefered_uris:
            components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] = [prefered_uris[uri_reduced]]
        else:
            best_vocab = find_best_lev_match(uri_reduced, list(prefered_uris.keys()))
            if best_vocab:
                components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] = [prefered_uris[best_vocab[1]]]
            # else:
            #     print(uri)

    
    for uri in components_to_reduce:
        uri_reduced = ""
        if "#" in uri:
            uri_reduced = "#".join(uri.split("#")[:-1])+"#"
        else:
        ...
        if uri_reduced in prefered_uris and prefered_uris[uri_reduced] in components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"]:
            # print("simple pref", uri, prefered_uris[uri_reduced], components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"])
            components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] = [prefered_uris[uri_reduced]]
        else:
            best_vocab = find_best_lev_match(uri_reduced, {prefUris:v for v in components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] for prefUris in vocabularies[v]["http://purl.org/vocab/vann/preferredNamespaceUri"]})
            if best_vocab:
                # print(uri, uri_reduced, best_vocab)
                components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] = [prefered_uris[best_vocab[1]]]

def find_best_lev_match(uri, vocabularies):

    best_match = (1_000_000_000, "")
    returnable = False #Can the tuple be returned as good value 
    for vocabulary in vocabularies:
        distance = Levenshtein.distance(uri, str.encode(vocabulary[1:-1]))
        if distance < best_match[0]:
            best_match = (distance, vocabulary)
            returnable = True
        elif distance == best_match[0]: # Two vocabularies have the same likelihood
            returnable = False

    if returnable and best_match[0] < 10:
        return best_match
    return None
...
if __name__ == "__main__":
    ...
    properties = retrieve_properties(sparql, vocabularies)
    adding_isDefinedBy(vocabularies, properties)
    remove_not_isDefinedBy(properties)

    classes = retrieve_classes(sparql, vocabularies)
    adding_isDefinedBy(vocabularies, classes)
    remove_not_isDefinedBy(classes)

    with open("./HomogenizedDataIsDefinedMandatoryButImprovedLevStyleVocabAssured.nt", "w", encoding="UTF-8") as f:
