...
def all_possible_namespace(namespace):
    namespaces = set([namespace])
    if namespace[:5] == "https":
        namespaces.add("http"+namespace[5:])
    else:
        namespaces.add("https"+namespace[4:])

    for namespace_added in list(namespaces):
        if namespace_added[-1] == "/":
            namespaces.add(namespace_added[:-1])
            namespaces.add(namespace_added[:-1]+"#")
        elif namespace_added[-1] == "#":
            namespaces.add(namespace_added[:-1])
            namespaces.add(namespace_added[:-1]+"/")
        else:
            namespaces.add(namespace_added+"/")
            namespaces.add(namespace_added+"#")
    
    for namespace_added in list(namespaces):
        namespaces.add(namespace_added.replace("/",""))
...
def retrieve_properties(sparql:SPARQLWrapper, vocabularies:dict):
    for vocabulary in vocabularies:
        ...
        interesting_relations_to_retrieve = {
            "http://www.w3.org/1999/02/22-rdf-syntax-ns#type" : (True, set([
                        "http://www.w3.org/1999/02/22-rdf-syntax-ns#type",
                        "http://purl.org/dc/elements/1.1/type"
                    ])),
            "http://www.w3.org/2000/01/rdf-schema#label" : (True, set([
            ...
            "http://www.w3.org/2000/01/rdf-schema#comment" : (True, set([
            ...
            "https://schema.org/rangeIncludes":(False, set([
            ...
            "http://www.w3.org/2000/01/rdf-schema#subPropertyOf":(False, set([
            ...
            "http://www.w3.org/2002/07/owl#equivalentProperty":(False, set([
                        "http://www.w3.org/2002/07/owl#equivalentProperty",
                        "http://www.w3.org/2002/07/owl#sameAs",
                        "http://www.w3.org/2004/02/skos/core#equivalentProperty",
                        "http://semanticscience.org/resource/equivalentTo",
                    ])),
            "http://www.w3.org/2002/07/owl#differentFrom":(False, set([
                        "http://www.w3.org/2002/07/owl#differentFrom",
                    ])),
            "http://www.w3.org/2002/07/owl#inverseOf":(False, set([
            ...
            # "http://www.w3.org/2000/01/rdf-schema#isDefinedBy":(True,set([
            #             "http://www.w3.org/2000/01/rdf-schema#isDefinedBy"
        ...
        for relation_to_retrieve in interesting_relations_to_retrieve:
            query = """
                ...
                SELECT DISTINCT ?propURI ?valueRelation {
                    ...
                    VALUES ?relationToRetrieve{
                        <"""+"> <".join(interesting_relations_to_retrieve[relation_to_retrieve][1])+""">
                    }
                    ?propURI rdf:type ?classProperty.
            ...
            response = sparql.queryAndConvert()
            for result in response["results"]["bindings"]:
                namespacevocab = all_possible_namespace(vocabulary)
                namespacething = ""
                thing = result["propURI"]["value"]
                if "#" in thing:
                    namespacething = "".join(result["propURI"]["value"].split("#")[:-1])
                else:
                    namespacething = "".join(result["propURI"]["value"].split("/")[:-1])
                sameNameSpace = (namespacething in namespacevocab)
                if sameNameSpace or not interesting_relations_to_retrieve[relation_to_retrieve][0]:
                    ...
                    if sameNameSpace and \
                        "http://www.w3.org/2000/01/rdf-schema#isDefinedBy" not in\
                          vocabularies[vocabulary]["Property"][result["propURI"]["value"]]:
                        vocabularies[vocabulary]["Property"][result["propURI"]["value"]]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] = \
                            [f"<{vocabulary}>"]
                
                if not sameNameSpace: 
                    print(namespacevocab, namespacething)
...
def retrieve_classes(sparql:SPARQLWrapper, vocabularies:dict):
    for vocabulary in vocabularies:
        ...
        interesting_relations_to_retrieve = {
            ...
            "http://www.w3.org/2000/01/rdf-schema#subClassOf":(False, set([
                        "http://www.w3.org/2000/01/rdf-schema#subClassOf",
                        "http://www.w3.org/2000/01/rdf-schema#subClasssOf",
                        "http://www.w3.org/2000/01/rdf-schema#subclassOf"
                    ])),
            "http://www.w3.org/2002/07/owl#equivalentClass":(False, set([
        ...
        for relation_to_retrieve in interesting_relations_to_retrieve:
            ...
            for result in response["results"]["bindings"]:
                ...
                namespacevocab = all_possible_namespace(vocabulary)
                namespacething = ""
                thing = result["classURI"]["value"]
                if "#" in thing:
                    namespacething = "".join(result["classURI"]["value"].split("#")[:-1])
                else:
                    namespacething = "".join(result["classURI"]["value"].split("/")[:-1])
                sameNameSpace = (namespacething in namespacevocab)
                if sameNameSpace or not interesting_relations_to_retrieve[relation_to_retrieve][0]:
                    ...
                    if sameNameSpace and \
                        "http://www.w3.org/2000/01/rdf-schema#isDefinedBy" not in\
                          vocabularies[vocabulary]["Class"][result["classURI"]["value"]]:
                        vocabularies[vocabulary]["Class"][result["classURI"]["value"]]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] = \
                            [f"<{vocabulary}>"]
