    ...
    for i in range(0,len(components_to_check), step):
        ...
        for uri in vocabularies_components:
            if len(vocabularies_components[uri]) == 1:
                ...
                best_vocab = find_best_lev_match(uri, vocabularies_components[uri])
                if best_vocab:
                    print(uri, best_vocab)
                    components[uri]["http://www.w3.org/2000/01/rdf-schema#isDefinedBy"] = [best_vocab[1]]
...
def find_best_lev_match(uri, vocabularies):
    ...
    if "#" in uri:
        ...
        uri_reduced = "/".join(uri.split("/")[:-1])
    uri_reduced = str.encode(uri_reduced)

    best_match = (1_000_000_000, "")
    returnable = False #Can the tuple be returned as good value 
    for vocabulary in vocabularies:
        distance = Levenshtein.distance(uri_reduced, str.encode(vocabulary[1:-1]))
        if distance < best_match[0]:
            best_match = (distance, vocabulary)
            returnable = True
        elif distance == best_match[0]: # Two vocabularies have the same likelyhood
            returnable = False
    print(uri_reduced)
    if returnable:
        return best_match
...
if __name__ == "__main__":
    ...
    with open("./HomogenizedDataIsDefinedMandatoryButImprovedLevStyle.nt", "w", encoding="UTF-8") as f:
